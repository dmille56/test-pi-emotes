...
def resize_png(in_path: str, final_size: str) -> None:
    """Resize PNGs locally using ImageMagick while preserving alpha."""
    # Expected format: "{w}x{h}" (square for pi-emotes).
    if not final_size or "x" not in final_size:
        return

    tmp_path = in_path + ".resized.tmp.png"
    cmd = [
        MAGICK_BIN,
        in_path,
        "-alpha",
...
def build_manifest(include_compact: bool) -> Dict[str, str]:
    ...
    if include_compact:
        manifest.update(
            {
                "compact/compact_1.png": "Compact variant 1 (compact visual style, face only): simplified/tighter rendering with fewer facial details and flatter shading; neutral expression; parrot stays perched; NO pose/framing change.",
                "compact/compact_2.png": "Compact variant 2 (compact visual style, face only): simplified/tighter rendering with slightly cleaner/less busy shading (still readable at 128px); neutral expression; parrot stays perched; NO pose/framing change.",
            }
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Requested output size passed to images.edit (minimum depends on model). e.g. 256x256",
    )
    parser.add_argument(
        "--final-size",
        default="128x128",
        help="Final square size to resize PNGs to locally (e.g. 128x128 for pi-emote sprites).",
    )
    ...
    parser.add_argument(
        "--include-compact",
        action="store_true",
        help="(deprecated) compact frames are generated by default; use --skip-compact to disable.",
    )
    parser.add_argument(
        "--skip-compact",
        action="store_true",
        help="Do not generate compact/compact_1.png and compact/compact_2.png.",
    )
    parser.add_argument(
        "--only",
        type=str,
        default="",
        help="Comma-separated list of relative PNG paths or glob patterns to generate (e.g. compact/compact_1.png,hi/*.png).",
    )
    ...
    manifest = build_manifest(include_compact=not args.skip_compact)
    ...
    if args.only and args.only.strip():
        ...
        print(f"[info] --only filtered frames: {before_n} -> {len(items)}")
        if not items:
            raise SystemExit(f"--only did not match any frames. patterns={patterns}")
    ...
    for rel_path, change_desc in items:
        ...
        # If the model requires a larger generation size than pi-emotes need,
        # resize down locally.
        try:
            if args.final_size and args.final_size != args.size:
                resize_png(out_path, args.final_size)
        except Exception as e:  # noqa: BLE001
            print(f"[warn] resize failed for {rel_path}: {e}. Keeping original output.")
